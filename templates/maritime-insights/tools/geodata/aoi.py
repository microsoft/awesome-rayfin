...
PLAN §4: this is a reusable asset. No coordinate, place name or site id belongs in a source file —
it all comes from config/aoi/<id>.json. The Kieler Förde is the first instance of this app, not the
app itself.

The AOI has two tiers (PLAN §4.1), and every helper here takes the tier as an argument rather than
assuming one:

  core   the photoreal box — LVermGeo SH DGM1, bDOM, LoD2, DOP20
  shell  the coarse horizon — Copernicus DEM, terrain only, and the reason a 41 km sensor horizon
         has real coastline to end on rather than the edge of the data
"""
...
def load_aoi(aoi_id: str = "kieler-foerde") -> dict[str, Any]:
