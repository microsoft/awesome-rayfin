...
def main() -> None:
    ...
    if len(windows) == 1:
        ...
        # 🔴 Mosaic. The sibling repo raised NotImplementedError here rather than ship untested
        # code for a case it did not have; this AOI has that case, so it is written and checked.
        # The tiles are pasted by GEOGRAPHIC position rather than concatenated in list order —
        # an hstack would look right for two tiles side by side and be silently wrong the moment
        # a third one sat above them.
        steps = {(round(m["lonStep"], 12), round(m["latStep"], 12)) for _, m in windows}
        if len(steps) != 1:
            raise RuntimeError(f"tiles disagree on pixel size: {steps}")
        lon_step, lat_step = steps.pop()
        west_out = min(m["lonWest"] for _, m in windows)
        east_out = max(m["lonEast"] for _, m in windows)
        north_out = max(m["latNorth"] for _, m in windows)
        south_out = min(m["latSouth"] for _, m in windows)
        out_w = int(round((east_out - west_out) / lon_step)) + 1
        out_h = int(round((north_out - south_out) / abs(lat_step))) + 1
        print(f"\nmosaic: {len(windows)} tiles -> {out_w} x {out_h}")
        window = np.full((out_h, out_w), np.nan, dtype=np.float32)
        for part, m in windows:
            c0 = int(round((m["lonWest"] - west_out) / lon_step))
            r0 = int(round((north_out - m["latNorth"]) / abs(lat_step)))
            window[r0:r0 + part.shape[0], c0:c0 + part.shape[1]] = part
            print(f"  placed {part.shape[1]}x{part.shape[0]} at row {r0}, col {c0}")
        gaps = int(np.isnan(window).sum())
        if gaps:
            # A gap means the tiles do not abut, which means the placement arithmetic or the
            # tile list is wrong. Never fill it — a seam of invented ground is exactly the kind of
            # defect that survives review because it looks like terrain.
            raise RuntimeError(f"mosaic left {gaps} cells unfilled — the tiles do not abut")
        meta = dict(windows[0][1])
        meta.update({"width": out_w, "height": out_h, "lonWest": west_out, "lonEast": east_out,
                     "latNorth": north_out, "latSouth": south_out,
