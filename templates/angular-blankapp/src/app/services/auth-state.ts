...
export class AuthState {
   ...
   *
   * Wrapped in a 30s timeout so a misbehaving auth provider can't leave
   * the user staring at a blank screen forever.
   */
  ...
  async signOut(): Promise<void> {
    try {
      await withTimeout(this.auth.signOut(), AUTH_TIMEOUT_MS, 'signOut');
    } catch (err) {
